...
const AutoCard = ({ car, adminOpt = false }) => {
  ...
  return (
    <>
      {images?.length > 0 && (
        <article className={`group m-2 max-w-sm overflow-hidden rounded-md ${AuctionId ? "bg-gray-100" : "bg-red-300"} transition-all duration-150 hover:bg-gray-200`}>
          <div className="relative">
...

            {
              ...
              >
                <p>
                  No hay vuelta atrás en esta acción, una vez que la publicación
                  es eliminado no podrá recuperarlo y{" "}
                  <b>"{brand + " " + model}"</b> será eliminado de nuestro base
                  de datos
                </p>
              </Modal>
            }
            {
              ...
              >
                <CarDetailForm creatingAuction={true} model={car} />
              </Modal>
            }
